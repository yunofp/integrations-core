...
from application.blueprints.repositories.processedRequestRepository import (
    ProcessedRequestsRepository,
)
from application.blueprints.utils.data_processing import (
    defineVariablesGrow,
    defineVariablesWealth,
    defineVariablesWork,
)
...
def test_should_process_sucess_grow_contract(service, mocker):
    ...
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.grow_response
    )
...
def test_should_return_correct_names_contract(service, mocker):
    ...
    contractService.processAllContracts()
    assert (
        formatFileNameSpy.spy_return == "[Integração] Contrato Grow - Strokes INC..docx"
    )
...
def test_should_return_correct_name_work_contract(service, mocker):
    ...
    requestId = zeev_responses.grow_response[0]["id"]
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.work_response
    )
    processedRequestRepository.findByRequestId = MagicMock(return_value=False)
    clickSignClient.createEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostGrow = MagicMock(return_value={"data": {"id": 1}})
    ...
    contractService.processAllContracts()
    assert (
        formatFileNameSpy.spy_return == "[Integração] Contrato Work - Strokes INC..docx"
    )
...
def test_should_process_sucess_wealth_contract(service, mocker):
    ...
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.wealth_response
    )
    processedRequestRepository.findByRequestId = MagicMock(return_value=False)
    clickSignClient.createEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostWealth = MagicMock(
        return_value={"data": {"id": 1}}
    )
    clickSignClient.addSignerToEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.addQualificationRequirements = MagicMock(
    ...
    spyInsert = mocker.spy(contractService, "_insertSuccessfullyProcessedRequest")
    spyClickSignClientPostWealth = mocker.spy(
        clickSignClient, "sendClickSignPostWealth"
    )
    ...
    implatationValue = combined_args[0].get(
        "Valor da Implantação", "Argument not found"
    )

    paymentDate = combined_args[0].get(
        "Data do Pagamento da Implantação", "Argument not found"
    )
    paymentMethod = combined_args[0].get(
        "Forma de Pagamento da Implantação", "Argument not found"
    )
    ...
    spyInsert.assert_called_once_with(
        requestId, "Wealth", [{"type": "Wealth", "id": 1}]
    )
    assert spyInsert.call_args == call(
        requestId, "Wealth", [{"type": "Wealth", "id": 1}]
    )
...
def test_should_process_sucess_grow_and_wealth_contract(service, mocker):
    ...
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.grow_response_wealth_response
    )
    processedRequestRepository.findByRequestId = MagicMock(return_value=False)
    clickSignClient.createEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostGrow = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostWealth = MagicMock(
        return_value={"data": {"id": 2}}
    )
    clickSignClient.addSignerToEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.addQualificationRequirements = MagicMock(
        return_value={"data": {"id": 1}}
    ...
    spyInsert.assert_called_once_with(
        requestId,
        "Grow & Wealth",
        [{"type": "Grow", "id": 1}, {"type": "Wealth", "id": 2}],
    )
    assert spyInsert.call_args == call(
        requestId,
        "Grow & Wealth",
...
def test_should_process_sucess_grow_and_wealth_contract_with_correct_variables(
    service, mocker
):
    contractService, zeevClient, processedRequestRepository, clickSignClient = service
...
def test_should_process_many_contract(service, mocker):
    ...
    manyContractsReponse = (
        list(zeev_responses.grow_response)
        + list(zeev_responses.wealth_response)
        + list(zeev_responses.grow_response_not_filled)
    )
...
def test_should_save_failed_grow_contract_request(service, mocker):
    ...
    assert expectedSaveRequest.get("status") == {
        "name": "error",
...
def test_retry_grow_contract_save_on_incomplete_contract(service, mocker):
    contractService, zeevClient, processedRequestRepository, clickSignClient = service
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.grow_response_not_filled
    )
    processedRequestRepository.findByRequestId = MagicMock(return_value=False)
    clickSignClient.createEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostGrow = MagicMock(return_value={"data": {"id": 1}})
...
def test_should_get_correct_clicksign_variables_sucess_grow_contract():
    clicksignVariables = defineVariablesGrow(
        zeev_responses.grow_response[0].get("formFields")
    )
...
def test_should_get_correct_clicksign_variables_sucess_wealth_contract():
    clicksignVariables = defineVariablesWealth(
        zeev_responses.wealth_response[0].get("formFields")
    )
...
def test_should_get_correct_clicksign_variables_success_work_contract():
    clicksignVariables = defineVariablesWork(
        zeev_responses.work_response[0].get("formFields")
    )
    ...
    assert (
        clicksignVariables.get("Email do Responsável")
        == "tarcisio.campos@yunofp.com.br"
    )
...
def test_should_process_retry_contract(service, mocker):
    ...
    zeevClient.getContractRequestById = MagicMock(
        return_value=zeev_responses.grow_response
    )
...
def test_should_not_process_retry_when_contract_is_not_fully_filled(service, mocker):
    ...
    zeevClient.generateZeevToken = MagicMock(return_value="token")
    zeevClient.getContractRequestById = MagicMock(
        return_value=zeev_responses.grow_response_not_filled[0]
    )
...
def test_should_save_invalid_work_type_contract_request(service, mocker):
    ...
    requestId = zeev_responses.grow_response[0]["id"]
    zeevClient.getContractsRequestsByDate = MagicMock(
        return_value=zeev_responses.invalid_work_type_response
    )
    processedRequestRepository.findByRequestId = MagicMock(return_value=False)
    clickSignClient.createEnvelope = MagicMock(return_value={"data": {"id": 1}})
    clickSignClient.sendClickSignPostGrow = MagicMock(return_value={"data": {"id": 1}})
    ...
    assert expectedSaveRequest.get("status") == {
        "name": "error",
        "description": "processContract | Invalid work type:1150",
    }
    spyInsert.assert_called_once_with(
        requestId, True, "processContract | Invalid work type:1150", "error", True
    )
    assert spyInsert.call_args == call(
        requestId, True, "processContract | Invalid work type:1150", "error", True
    )
...
def test_should_not_notificate_when_occurs_error_on_integrations_api(service, mocker):
    ...
    clickSignClient.addAuthRequirements = MagicMock(
        side_effect=Exception("Auth requirement error")
    )
    clickSignClient.activateEnvelope = MagicMock(return_value={})
    clickSignClient.notificateEnvelope = MagicMock(return_value={})
    ...
    assert spyInsertFailed.call_args == call(
        1150, True, "Auth requirement error", "error", True
    )
