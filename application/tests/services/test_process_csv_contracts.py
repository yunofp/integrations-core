    ...
    expected_contract_call = call(
        {
            ...
            "implantation": 250.0,
            "first_implantation_payment_date": utils_test_convert_to_utc_date(
                "10/01/2018"
            ),
...
def assert_entry_insertion(spyInsertEntry):
    ...
    expected_length = 86
    assert (
        len(call_args) == expected_length
    ), f"Expected list length {expected_length}, but got {len(call_args)}"
