...
class ContractsRepository:
    ...
    def get_last_inserted_contract_code(self):
        last_inserted_contract_code = self.collection.find_one(
            sort=[("_id", -1)], projection={"code": 1, "_id": 0}
        )
        return (
            last_inserted_contract_code["code"] if last_inserted_contract_code else None
        )

    def get_last_id(self):
        last_inserted_contract_id = self.collection.find_one(
