...
class ClicksignClient:
    ...
    def addSignerToEnvelope(
        ...
        phoneNum=None,
        email=None,
        signatureRequest="whatsapp",
        documentSigned="whatsapp",
    ):
        ...
        data = {"data": {"type": "signers", "attributes": attributes}}

        response = requests.post(url, json=data, headers=self.headers)

        return response.json()

    def addQualificationRequirements(
        self, envelopeId, signerId, documentId, role="contractor"
    ):
    ...
    def activateEnvelope(self, envelopeId):
        try:
            ...
            csResponse = json.loads(json_response)
            logger.error(
                "activateEnvelope: Error while sending document: "
                + str(e)
                + " "
                + str(csResponse)
            )
    ...
    def notificateEnvelope(self, envelopeId):
        try:
            ...
            data = {
                "data": {"type": "notifications", "attributes": {"message": "YUNO CS"}}
            }
            ...
            csResponse = json.loads(json_response)
            logger.error(
                "notificateEnvelope: Error while sending document: "
                + str(e)
                + " "
                + str(csResponse)
            )
