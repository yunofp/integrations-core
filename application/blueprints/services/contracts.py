...
class ContractsService:
    ...
    def processContract(self, requestId, contractValues):
        ...
        if workTypeFormatted == "Grow":
            ...
            contractVariablesWealth = data_processing.defineVariablesWealth(contractValues, implantationValue,
    ...
    def _definePhoneNumber(self, contractVariables):
        if self.config.get('PHONE_NUMBER_DEBUG'):
            ...
            phoneNum = formatting.clearPhoneNum(
                contractVariables.get("Telefone do Titular") or contractVariables.get("Telefone da Empresa"))
        return phoneNum
    ...
    def _addSignersRequirements(self, envelopeId, contractVariables, phoneNum, documentId):
        ...
        clientCpf = formatting.formatCpf(
            contractVariables.get("CPF do Responsável") or contractVariables.get("cpfDoResponsavel"))
        ...
        clientSignerResponse = self.clickSignClient.addSignerToEnvelope(envelopeId, clientName, clientCpf,
                                                                        clientBirthdate, phoneNum, clientEmail)
        ...
        qualificationRequirementsResponse = self.clickSignClient.addQualificationRequirements(envelopeId, signerId,
                                                                                              documentId)

        qualificationRequirementsId = qualificationRequirementsResponse.get('data', {}).get('id')

        if not qualificationRequirementsId:
            logger.error("processContract | data:", " envelopeId: ", envelopeId, " signerId: ", signerId,
                         " documentId: ", documentId)
            raise Exception("processContract | Error while creating qualification requirements:" + str(
        ...
        jgvSignerResponse = self.clickSignClient.addSignerToEnvelope(envelopeId, jgvName, None, None, jgvPhone,
                                                                     jgvEmail, 'email', 'email')
        ...
        jgvQualificationRequirementsResponse = self.clickSignClient.addQualificationRequirements(envelopeId,
                                                                                                 jgvSignerId,
                                                                                                 documentId)
        ...
        if not jgvQualificationRequirementsId:
            raise Exception("processContract | Error while creating jgv qualification requirements:" + str(
                jgvQualificationRequirementsId))
    ...
    def _insertSuccessfullyProcessedRequest(self, requestId, serviceType, documents):
        try:
            ...
            logger.error("_insertSuccessfullyProcessedRequest | Error inserting successfully processed request:" + str(
                requestId), exc_info=True)
    ...
    def _updateSuccessfullyProcessedRequest(self, requestId, serviceType, documentsData):
        try:
            ...
            logger.error(
                "_updateSuccessfullyProcessedRequest | Error updating successfully processed request:" + requestId,
                exc_info=True)
    ...
    def _updateFailedProcessedRequest(self, requestId, statusName, statusDescription):
        try:
            ...
            logger.error("_updateFailedProcessedRequest | Error updating successfully processed request:" + requestId,
                         exc_info=True)
    ...
    def _insertFailedProcessedRequest(self, requestId, tryAgain, errorMessage, statusName=None, validNewClient=False):
        try:
            ...
            logger.error("_insertFailedProcessedRequest | Error inserting failed processed request:" + requestId,
                         exc_info=True)
        ...
        try:
            ...
            for processedRequest in processedRequestsRetries:
                ...
                try:
                    ...
                    self._updateFailedProcessedRequest(processedRequest['requestId'], 'Error',
    ...
    def processAllContracts(self):
        ...
        try:
            ...
            contractsRequests = self.zeevClient.getContractsRequestsByDate(zeevToken, formattedStartDate,
                                                                           formattedEndDate)
        ...
        for contractRequest in contractsRequests:
            ...
            if not isContractCompletelyFilledToProcess:
                ...
                self._insertFailedProcessedRequest(requestId, True, 'Contract not completely filled to process',
                                                   'waitingFill', True)
                continue
                    ...
                    for index, row in df.iterrows():
                        ...
                        try:
                            ...
                            if not is_code_null and not is_name_null:
                                ...
                                if not cod_exists:
                                    profile_dict = pandas_processement.create_profile_dict(index, df,
                                    ...
                                    contract_dict = pandas_processement.create_contract_dict(index,
                                                                                             pandas_processement.get_cell_content(
                                                                                                 df, index, 'COD'), df,
                                                                                             profile_id)

                                    contract_id = self.contract_repository.insert_one(contract_dict, session)
                                    self.iterate_by_row(df, index, contract_id,
                                                        pandas_processement.get_cell_content(df, index, 'COD'), session)
