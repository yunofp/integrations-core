...
class ContractsService:
    ...
    def processContract(self, requestId, contractValues):
        ...
        if workTypeFormatted == "Grow":
            ...
            documentId = self._processContractSteps(
                contractVariables, envelopeId, workTypeFormatted
            )
            ...
            contractVariablesWealth = data_processing.defineVariablesWealth(
                contractValues, implantationValue, paymentDate, paymentMethod
            )
            documentIdGrow = self._processContractSteps(
                contractVariablesGrow, envelopeId, "Grow"
            )
            newEnvelope = self.clickSignClient.createEnvelope(str(requestId) + "Wealth")
            newEnvelopeId = newEnvelope.get("data", {}).get("id")
            documentIdWealth = self._processContractSteps(
                contractVariablesWealth, newEnvelopeId, "Wealth"
            )
            documents.extend(
                [
                    {"type": "Grow", "id": documentIdGrow},
                    {"type": "Wealth", "id": documentIdWealth},
                ]
            )
    ...
    def _definePhoneNumber(self, contractVariables):
        if self.config.get("PHONE_NUMBER_DEBUG"):
            ...
            phoneNum = formatting.clearPhoneNum(
                contractVariables.get("Telefone do Titular")
                or contractVariables.get("Telefone da Empresa")
            )
        return phoneNum
    ...
    def _processContractSteps(self, contractVariables, envelopeId, contractType):
        try:
            ...
            logger.info(
                "_processContractSteps | sending contract to phoneNum:" + str(phoneNum)
            )
            ...
            if contractType == "Grow":
                ...
                growResponse = self.clickSignClient.sendClickSignPostGrow(
                    contractVariables, envelopeId, filename
                )
                documentId = growResponse.get("data", {}).get("id")

                if not documentId:
                    raise Exception(
                        "processContract | Error while creating document:"
                        + str(growResponse)
                    )

            elif contractType == "Wealth":
                wealthResponse = self.clickSignClient.sendClickSignPostWealth(
                    contractVariables, envelopeId, filename
                )
                ...
                workResponse = self.clickSignClient.sendClickSignPostWork(
                    contractVariables, envelopeId, filename
                )
            ...
            self._addSignersRequirements(
                envelopeId, contractVariables, phoneNum, documentId
            )
    ...
    def _addSignersRequirements(
        self, envelopeId, contractVariables, phoneNum, documentId
    ):
        clientName = contractVariables.get("nomeCompletoDoTitular")
        clientCpf = formatting.formatCpf(
            contractVariables.get("CPF do Responsável")
            or contractVariables.get("cpfDoResponsavel")
        )
        clientEmail = contractVariables.get(
            "Email de Contato"
        ) or contractVariables.get("Email")

        clientBirthdate = formatting.formatBirthdate(
            contractVariables.get("Data de Nascimento do Responsável")
        )

        clientSignerResponse = self.clickSignClient.addSignerToEnvelope(
            envelopeId, clientName, clientCpf, clientBirthdate, phoneNum, clientEmail
        )
        ...
        if not signerId:
            raise Exception(
                "processContract | Error while creating signer:"
                + str(clientSignerResponse)
            )

        qualificationRequirementsResponse = (
            self.clickSignClient.addQualificationRequirements(
                envelopeId, signerId, documentId
            )
        )

        qualificationRequirementsId = qualificationRequirementsResponse.get(
            "data", {}
        ).get("id")
        ...
        if not qualificationRequirementsId:
            ...
            raise Exception(
                "processContract | Error while creating qualification requirements:"
                + str(qualificationRequirementsResponse)
            )
        ...
        jgvSignerResponse = self.clickSignClient.addSignerToEnvelope(
            envelopeId, jgvName, None, None, jgvPhone, jgvEmail, "email", "email"
        )
        ...
        if not jgvSignerId:
            raise Exception(
                "processContract | Error while creating jgv signer:"
                + str(jgvSignerResponse)
            )
        ...
        jgvQualificationRequirementsId = jgvQualificationRequirementsResponse.get(
            "data", {}
        ).get("id")

        if not jgvQualificationRequirementsId:
            raise Exception(
                "processContract | Error while creating jgv qualification requirements:"
                + str(jgvQualificationRequirementsId)
            )
    ...
    def _insertSuccessfullyProcessedRequest(self, requestId, serviceType, documents):
        try:
            status = {"name": "send", "descritpion": "delivered"}
            ...
            logger.error(
                "_insertSuccessfullyProcessedRequest | Error inserting successfully processed request:"
                + str(requestId),
                exc_info=True,
            )

    def _updateSuccessfullyProcessedRequest(
        self, requestId, serviceType, documentsData
    ):
        try:
            status = {"name": "send", "descritpion": "delivered"}
            self.processedRequestRepository.updateOne(
                requestId,
                {
                    "tryAgain": False,
                    "type": serviceType,
            ...
            logger.error(
                "_updateSuccessfullyProcessedRequest | Error updating successfully processed request:"
                + requestId,
                exc_info=True,
            )
    ...
    def _updateFailedProcessedRequest(self, requestId, statusName, statusDescription):
        try:
            status = {"name": statusName, "descritpion": statusDescription}
            self.processedRequestRepository.updateOne(
                requestId, {"status": status, "updatedAt": datetime.now(timezone.utc)}
            )
        except Exception as e:
            logger.error(
                "_updateFailedProcessedRequest | Error updating successfully processed request:"
                + requestId,
                exc_info=True,
            )

    def _insertFailedProcessedRequest(
        self, requestId, tryAgain, errorMessage, statusName=None, validNewClient=False
    ):
        try:
            document = {
                "tryAgain": tryAgain,
            ...
            if statusName:
                document["status"] = {"name": statusName, "description": errorMessage}

            self.processedRequestRepository.insertOne(document)
        except Exception as e:
            logger.error(
                "_insertFailedProcessedRequest | Error inserting failed processed request:"
                + requestId,
                exc_info=True,
            )
    ...
    def runTryAgain(self):
        try:
            ...
            for processedRequest in processedRequestsRetries:
                logger.info(
                    "runTryAgain | started to process contract: "
                    + str(processedRequest)
                )
                requestId = processedRequest.get("requestId")
                contractRequest = self.zeevClient.getContractRequestById(
                    token, requestId
                )

                if not contractRequest:
                    logger.warn(
                        "runTryAgain | Contract request not found: " + str(requestId)
                    )
                    continue

                contractValues = contractRequest[0]["formFields"]

                isContractCompletelyFilledToProcess = data_processing.findByName(
                    contractValues, "valorDoFEE"
                )

                if not isContractCompletelyFilledToProcess:
                    logger.info(
                        "runTryAgain | Contract not completely filled to process: "
                        + str(requestId)
                    )
                    continue

                try:
                    serviceType, documents = self.processContract(
                        requestId, contractValues
                    )
                    self._updateSuccessfullyProcessedRequest(
                        processedRequest["requestId"], serviceType, documents
                    )
                except Exception as e:
                    self._updateFailedProcessedRequest(
                        processedRequest["requestId"],
                        "Error",
                        "Retry Process error " + str(e),
                    )
                    logger.error(
                        "runTryAgain | Error processing contract: " + str(e),
                        exc_info=True,
                    )
                    continue
            ...
            logger.error(
                "runTryAgain | Error running try again: " + str(e), exc_info=True
            )
    ...
    def processAllContracts(self):
        ...
        try:
            ...
            contractsRequests = self.zeevClient.getContractsRequestsByDate(
                zeevToken, formattedStartDate, formattedEndDate
            )

        except requests.exceptions.RequestException as e:
            logger.error(
                "processAllContracts | Error during getting contracts:" + str(e),
                exc_info=True,
            )
        ...
        logger.info(
            "processAllContracts | starting to process contracts founds in date: "
            + formattedStartDate
        )
        ...
        for contractRequest in contractsRequests:
            ...
            if alreadyExists:
                logger.info(
                    "processAllContracts | Contract request already exists:"
                    + str(requestId)
                )
                continue
            ...
            if not isContractCompletelyFilledToProcess:
                logger.info(
                    "processAllContracts | Contract not completely filled to process:"
                    + str(requestId)
                )
                self._insertFailedProcessedRequest(
                    requestId,
                    True,
                    "Contract not completely filled to process",
                    "waitingFill",
                    True,
                )
                continue
            ...
            try:
                ...
                self._insertSuccessfullyProcessedRequest(
                    requestId, serviceType, documents
                )
            except Exception as e:

                self._insertFailedProcessedRequest(
                    requestId, True, e.__str__(), "error", True
                )
                logger.error(
                    "processAllContracts | Error processing contract:" + str(requestId),
                    exc_info=True,
                )
    ...
    def iterate_by_row(self, df, index, contract_id, code, session):
        ...
        for column_index in range(start_column, total_columns, step):
            ...
            mrr_paid_value = pandas_processement.get_cell_content(
                df, index, paid_mrr_index
            )
                ...
                with session.start_transaction(max_commit_time_ms=1200000):
                    for index, row in df.iterrows():
                        ...
                        if not readyStart:
                            continue
                        try:
                            is_code_null = pd.isnull(
                                pandas_processement.get_cell_content(df, index, "COD")
                            )
                            is_name_null = pd.isnull(
                                pandas_processement.get_cell_content(
                                    df, index, "Nome do Cliente"
                                )
                            )

                            if not is_code_null and not is_name_null:
                                cod_exists = self.cod_already_exists(
                                    pandas_processement.get_cell_content(
                                        df, index, "COD"
                                    )
                                )
                                if not cod_exists:
                                    profile_dict = (
                                        pandas_processement.create_profile_dict(
                                            index,
                                            df,
                                            pandas_processement.get_cell_content(
                                                df, index, "COD"
                                    ...
                                    profile_id = self.ProfileRepository.insert_one(
                                        profile_dict, session
                                    )
                                    profile_id = profile_id.inserted_id

                                    contract_dict = (
                                        pandas_processement.create_contract_dict(
                                            index,
                                            pandas_processement.get_cell_content(
                                                df, index, "COD"
                                            ),
                                    ...
                                    contract_id = self.contract_repository.insert_one(
                                        contract_dict, session
                                    )
                                    self.iterate_by_row(
                                        df,
                                        index,
                            ...
                            if is_code_null and not is_name_null:
                                ...
                                if not cod_exists:
                                    ...
                                    profile_inserted = (
                                        self.ProfileRepository.insert_one(
                                            profile_dict, session
                                        )
                                    )
                                    ...
                                    contract_dict = (
                                        pandas_processement.create_contract_dict(
                                            index, new_cod, df, profile_inserted_id
                                        )
                                    )
                                    contract_id = self.contract_repository.insert_one(
                                        contract_dict, session
                                    ...
                                    self.iterate_by_row(
                                        df, index, contract_id, new_cod, session
                                    )

                        except Exception as e:
                            logger.info(
                                "insert_contracts | Row Processing Exception | "
                                + str(e)
                            )
                            raise
