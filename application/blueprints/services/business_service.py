...
class BusinessService:
    ...
    def __init__(self, contract_repository, entries_repository, goalsRepository, indicationsRepository):
    ...
    def calculate_mrr_by_year_group_by_month(self, year, type, goal) -> tuple[dict[Any, int], dict[str, int | Any]] | \
        ...
        months_mrr = entries_data_frame.groupby('month', observed=False)['value'].sum().reindex(months_list,
    ...
    def calculate_implantation_by_year_group_by_month(self, year, type, goal) -> tuple[dict[Any, int], dict[
        str, int | Any]] | tuple[Any, dict[str, int | Literal[0] | Any]]:
        months_list = date.get_months_list()
        ...
        contracts_data_frame['month'] = contracts_data_frame['first_implantation_payment_date'].dt.strftime(
            '%b').str.upper()
        contracts_data_frame['month'] = pd.Categorical(contracts_data_frame['month'], categories=months_list,
                                                       ordered=True)

        months_mrr = contracts_data_frame.groupby('month', observed=False)['implantation'].sum().reindex(months_list,
                                                                                                         fill_value=0).to_dict()

        total = sum(months_mrr.values())
        return months_mrr, {"goal": goal, "actual": total}

    def calculate_aum_estimated_by_year_group_by_month(self, year, type, goal) -> tuple[
        Any, dict[str, int | Literal[0] | Any]]:
        months_list = date.get_months_list()
        ...
        months_mrr = contracts_data_frame.groupby('month', observed=False)['estimated'].sum().reindex(months_list,
                                                                                                      fill_value=0).to_dict()
        total = sum(months_mrr.values())
        return months_mrr, {"goal": goal, "actual": total}
    ...
    def get_new_business_values(self, year=None, type='GROW') -> dict:
        ...
        implantation_by_months, implantation_actual = self.calculate_implantation_by_year_group_by_month(year, type,
                                                                                                         imp_goal[
                                                                                                             'value'])
        complete_implantation_data = {"months": implantation_by_months, "goal": implantation_actual}
        new_business["imp"] = {"label": 'Novo IMP', "year": year, **complete_implantation_data}

        implantation_by_months, implantation_actual = self.calculate_aum_estimated_by_year_group_by_month(year, type,
                                                                                                          imp_goal[
                                                                                                              'value'])
        ...
        now = datetime.now()
        new_business['current_month']["new_clients"] = {'label': 'Novos Clientes',
                                                        'value': self.get_new_clients_count_by_month(now)}
        new_business['current_month']["indications"] = {'label': 'Indicações',
