...
class BusinessService:
  ...
  def __init__(self, contract_repository, entriesRepository, goalsRepository, indicationsRepository):
  ...
  def get_new_business_values(self, year=None, type='GROW') -> dict:
    ...
    now = datetime.now()
    new_business['current_month']["new_clients"] = {'label': 'Novos Clientes', 'value': self.get_new_clients_count_by_month(now)}
    new_business['current_month']["indications"] = {'label': 'Indicações', 'value': self.get_indications_count_by_month(now)}
    return new_business
